...
const FeedingList = ({id,keeperID,animalData}) => {
    ...
    return ( 
        <div className='feeding-list'>
           <h3>Feedings</h3>
           <button onClick={handleAddFeedingClick}>Add Feeding</button>
           {error && <div>{error}</div>}
           {isLoading && <div>Loading...</div>}
           {data && 
           data.map(feeding=> (
            <div className="feeding container text-center" key={feeding.id}>
                <div className="row row-cols-3">
                    <div className="col">{feeding.date}</div>
                    <div className="col">{feeding.keeperFirstName}</div>
                    <div className="col"><button onClick ={()=>handleDeleteClick(feeding.id)}>Delete</button></div>
                </div>
            </div>
           )
           )}       
